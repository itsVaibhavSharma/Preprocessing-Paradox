...
class OptimizedModelTrainer:
    ...
    def train_all_models(self):
        ...
        trained_models_count = len(all_combinations) - len(remaining_combinations)
        
        for idx, config in enumerate(remaining_combinations, 1):
            print(f"\n{'='*70}")
            print(f"Training Model {trained_models_count + idx}/{len(all_combinations)}")
            print(f"{'='*70}")
    ...
    def train_single_model(self, model_name, seg_method, input_method, aug_type, model_type):
        """Train single model with mixed precision, epoch resume and multiple seeds"""
        ...
        # Create optimized data loaders (shared across seeds for efficiency)
        ...
        for seed in Config.RANDOM_SEEDS:
            ...
            seed_model_name = f"{model_name}_seed{seed}"
            model_path = os.path.join(Config.OUTPUT_BASE, 'models', f'{seed_model_name}.pth')
            checkpoint_path = os.path.join(Config.OUTPUT_BASE, 'checkpoints', f'{seed_model_name}_checkpoint.pth')
            seed_metrics_path = os.path.join(Config.OUTPUT_BASE, 'metrics', f'{seed_model_name}_metrics.json')
            
            # If this seed was already fully trained and evaluated, skip it completely
            if os.path.exists(seed_metrics_path) and os.path.exists(model_path):
                print(f"  Seed {seed} already fully trained. Loading cached metrics...")
            ...
            try:
                ...
                training_state = self.progress_tracker.get_training_state(seed_model_name)
                ...
                for epoch in range(start_epoch, Config.EPOCHS):
                    try:
                        ...
                        self.progress_tracker.save_training_state(seed_model_name, epoch + 1, best_val_loss)
                ...
                training_time = time.time() - seed_start_time
                ...
                history_path = os.path.join(Config.OUTPUT_BASE, 'training_history', f'{seed_model_name}_history.json')
                with open(history_path, 'w') as f:
                    json.dump(history, f, indent=4)
                self.plot_training_curves(seed_model_name, history)
                ...
                seed_results.append(seed_metric)
                
                # Cache the metrics so we can skip this seed on restart
                os.makedirs(os.path.join(Config.OUTPUT_BASE, 'metrics'), exist_ok=True)
                with open(seed_metrics_path, 'w') as f:
                    json.dump(seed_metric, f, indent=4)
                
                # Copy the first seed's model to be the main model for evaluation later
                if seed == Config.RANDOM_SEEDS[0]:
                    main_model_path = os.path.join(Config.OUTPUT_BASE, 'models', f'{model_name}.pth')
                    import shutil
                ...
                print(f"Error training {seed_model_name}: {e}")
                traceback.print_exc()
                continue
        ...
        if len(seed_results) > 0:
            ...
            avg_prec = np.mean([r['test_precision'] for r in seed_results])
            std_prec = np.std([r['test_precision'] for r in seed_results])
            avg_f1 = np.mean([r['test_f1_score'] for r in seed_results])
            std_f1 = np.std([r['test_f1_score'] for r in seed_results])
            total_time = sum([r['training_time_seconds'] for r in seed_results])
            ...
            result = {
                ...
                'test_accuracy_mean': float(avg_acc),
                'test_accuracy_std': float(std_acc),
                'test_precision_mean': float(avg_prec),
                'test_precision_std': float(std_prec),
                'test_f1_mean': float(avg_f1),
                'test_f1_std': float(std_f1),
                'seed_runs': seed_results
            }
            
            print(f"\n{'='*50}")
            print(f"Aggregated Results ({len(seed_results)} runs): acc={avg_acc:.4f}±{std_acc:.4f} f1={avg_f1:.4f}±{std_f1:.4f}")
